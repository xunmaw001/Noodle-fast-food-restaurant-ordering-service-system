                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <div class="table-responsive mb-3">
                            <div class="col-sm-12">
                                        ...
                                        <label>
                                            菜品单价
                                            <input type="text" id="moneyStartSearch" class="form-control form-control-sm"
                                                   onchange="moneyStartSearchChickValue(this)" placeholder="开始菜品单价" aria-controls="tableId">
                                        </label>
                                        <label>
                                            -
                                        </label>
                                        <label>
                                            <input type="text" id="moneyEndSearch" class="form-control form-control-sm"
                                                   onchange="moneyEndSearchChickValue(this)" placeholder="结束菜品单价" aria-controls="tableId">
                                        </label>
                                                                
                                <button onclick="search()" type="button" class="btn btn-primary">查询</button>
                                </br>
                                <button onclick="add()" type="button" class="btn btn-success 新增">添加</button>
                                <button onclick="diancan()" type="button" class="btn btn-success 点餐">点餐</button>
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th>菜品图片</th>
                                    <th onclick="sort('shangpinContent')">菜品描述</th>
                                    <th>操作</th>
                                </tr>
                                </thead>
                                <tbody>
...
<script>
    ...
    function search() {
        ...
        searchForm.moneyStartSearch = moneyStartSearch;
        var moneyEndSearch = $('#moneyEndSearch').val();
        if( moneyEndSearch  == 0){
                moneyEndSearch ='';
        }
        searchForm.moneyEndSearch = moneyEndSearch;
    ...
    function getDataList() {
        http("shangpin/page", "GET", {
            ...
            sort: sortColumn,
            order: sortOrder,
            moneyStartSearch: searchForm.moneyStartSearch,
            moneyEndSearch: searchForm.moneyEndSearch,
    }, (res) => {
            if(res.code == 0
            )
            {
    ...
    function setDataRow(item, number) {
        ...
        var shangpinContentCell = document.createElement('td');

        //如果图文详情包含图片
        if (item.shangpinContent && item.shangpinContent.indexOf('img') != -1) {
            //暂时只考虑图片+文字 和 文字+图片的情况
            var beginIndex = item.shangpinContent.indexOf('<img');
            var endIndex = item.shangpinContent.indexOf('>');
                shangpinContentCell.innerHTML = item.shangpinContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
        } else {
            if (item.shangpinContent != null && item.shangpinContent != "" && item.shangpinContent.length >= 11) {
                    shangpinContentCell.innerHTML = item.shangpinContent.substring(0, 10) + "...";
            } else {
                    shangpinContentCell.innerHTML = item.shangpinContent;
            }
        }

        row.appendChild(shangpinContentCell);
    ...
    //搜素输入检查
                function moneyStartSearchChickValue(e){
                    ...
                    var reg=/^[1-9][0-9]*$/;
                    if(!reg.test(this_val) && e.value != "" && e.value != null){
                        e.value = "";
                        alert("输入不合法");
                        return false;
                    }
